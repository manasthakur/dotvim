...
silent! unlet s:style s:seoul_background

" 1. If g:seoul_background is found
if exists('g:seoul_background')
  let s:seoul_background = g:seoul_background
  if s:seoul_background >= 233 && s:seoul_background <= 239
    let s:style = 'dark'
  elseif s:seoul_background >= 252 && s:seoul_background <= 256
    let s:style = 'light'
  else
    unlet s:seoul_background
  endif
endif

if !exists('s:style')
  " 2. If g:colors_name is NOT 'seoul' -> dark version
  if s:colors_name != 'seoul'
    let s:style = 'dark'
...
" Popup menu
call s:hi('Pmenu', [s:dark_bg + 1, 238], [252, 252])
call s:hi('PmenuSel', [s:dark_fg, s:dark_fg], [95, 95])
call s:hi('PmenuSbar', ['', ''], [251, 251])
call s:hi('PmenuThumb', ['', ''], [65, 65])

call s:hi('Search', [s:dark_fg, 255], [24, 74])
call s:hi('IncSearch', [254, 254], [s:dark_bg + 1, 238])
...
call s:hi('WildMenu', [95, 95], [187, 187])
...
let g:seoul_current_fg = [s:dark_fg, s:light_fg][s:style_idx]
let g:seoul_current_bg = [s:dark_bg, s:light_bg][s:style_idx]
